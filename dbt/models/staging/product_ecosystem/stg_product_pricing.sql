...
with source as (
    select * from {{ source('bronze', 'product_pricing_cdc') }}
),
...
parsed as (
    select
        ...
        (cdc_payload->'payload'->'after'->>'PricingID')::bigint as pricing_i_d,
        ...
        (cdc_payload->'payload'->'after'->>'Price')::numeric as price,
        -- CDC Metadata
