...
with source as (
    select * from {{ source('bronze', 'product_set_cdc') }}
),
...
parsed as (
    select
        ...
        (cdc_payload->'payload'->'after'->>'ComponentProductID')::bigint as component_product_i_d,
        ...
        (cdc_payload->'payload'->'after'->>'SetComponentsQty')::integer as set_components_qty,
        -- CDC Metadata
