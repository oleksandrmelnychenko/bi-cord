...
with source as (
    select * from {{ source('bronze', 'product_image_cdc') }}
),
...
parsed as (
    select
        ...
        (cdc_payload->'payload'->'after'->>'ImageUrl')::text as image_url,
        ...
        (cdc_payload->'payload'->'after'->>'IsMainImage')::boolean as is_main_image,
        -- CDC Metadata
